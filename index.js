    ...
    .then((answer) => {
      switch (answer.options) {
        case "View All Departments":
          databaseOps.getAllDepartments().then((data) => {
            console.table(data[0]);
            optionMenu();
          });
          break;
        case "View All Roles":
          databaseOps.getAllRoles().then((data) => {
            console.table(data[0]);
            optionMenu();
          });
          break;
        case "View All Employees":
          databaseOps.getAllEmployees().then((data) => {
            console.table(data[0]);
            optionMenu();
          });
          break;
...
const promptRole = () => {
  inquirer
    ...
    .then((answers) => {
      databaseOps.addRole(answers.title, answers.salary, answers.department_id);
      optionMenu();
    });
};
...
const promptUpdateEmployee = () => {
  const employeeIds = [];
  databaseOps.getEmployeeData().then((employeeData) => {
    ...
    inquirer
      ...
      .then((answers) => {
        const employee = employeeData[0].find(({ id }) => id === answers.id);
        switch (answers.property) {
          case "First Name":
            inquirer
              ...
              .then((input) => {
                databaseOps.updateEmployeeFirstName(
                  input.firstName,
                  employee.id
                );
                optionMenu();
          ...
          case "Manager ID":
            inquirer
              ...
              .then((input) => {
                databaseOps.updateEmployeeManagerId(
                  input.managerId,
                  employee.id
                );
                optionMenu();
...
const promptViewEmployeesByManager = () => {
  databaseOps.getManagers().then((managers) => {
    const managerIds = [];
    ...
    inquirer
      ...
      .then((managerId) => {
        databaseOps
          .viewEmployeesByManager(managerId.manager)
...
const promptDeleteRole = () => {
  databaseOps.getRoleNames().then((roles) => {
    const roleIds = [];
...
const promptDeleteDepartment = () => {
  databaseOps.getAllDepartments().then((departments) => {
    const departmentIds = [];
    departments[0].map((department) =>
...
const promptDeleteEmployee = () => {
  databaseOps.getAllEmployees().then((employees) => {
    const employeeIds = [];
